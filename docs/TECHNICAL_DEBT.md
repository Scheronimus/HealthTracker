...
- CSV parsing and merge utilities remain tested but are hidden from the production interface as of v1.4.0. Either remove them or restore them only after defining a round-trippable advanced data-exchange contract covering module scope, stable headers, dates, units, collisions, validation previews, accessibility, localization, and spreadsheet compatibility.
