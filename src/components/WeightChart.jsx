...
import { chartGeometry, filterBySpan } from '../utils/chart.js'
      ...
      <div className="chart-wrap">
        <svg className="chart-svg" viewBox={`0 0 ${WIDTH + PAD.left + PAD.right} ${HEIGHT + PAD.top + PAD.bottom}`} role="img" aria-label={t('chartDescription', { count: points.length })}>
          ...
          <g transform={`translate(${PAD.left} ${PAD.top})`}>
            ...
            {points.map((point) => <g key={point.id} className="chart-point" tabIndex="0" role="button" aria-label={`${formatDateTime(point.timestamp, language)}, ${point.value.toFixed(1)} kg`} onFocus={() => setActive(point)} onBlur={() => setActive(null)} onClick={() => setActive(active?.id === point.id ? null : point)}>
              <circle className="point-target" cx={point.x} cy={point.y} r="16" />
              <circle className="point-dot" cx={point.x} cy={point.y} r={active?.id === point.id ? 6 : 4} />
            </g>)}
