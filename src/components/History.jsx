...
export function History({ measurements, language, onEdit, t }) {
  return <section className="history">
    ...
    <div className="entry-list">{measurements.map((item, index) => {
      ...
      return <button className="entry entry-link card" type="button" key={item.id} onClick={() => onEdit(item)} aria-label={`${t('editAction')}: ${formatDate(item.timestamp, language)}, ${item.value.toFixed(1)} kg`}>
        <div><time dateTime={item.timestamp}>{formatDate(item.timestamp, language)}</time>{item.note && <p>{item.note}</p>}</div>
