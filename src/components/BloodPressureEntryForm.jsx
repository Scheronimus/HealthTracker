...
export function BloodPressureEntryForm({ editing, preset, measurements, onSave, onDelete, t }) {
  ...
  function submit(event) {
    ...
    if (isBloodPressureDayFull(measurements, date, editing?.id)) { setError(t('bpDayFull')); return }
    if (hasNearbyBloodPressureReading(measurements, timestamp, editing?.id)) { setError(t('bpSameSession')); return }
    const next = createBloodPressureMeasurement({ timestamp, ...values })
  ...
  const timeField = <label>{t('time')}<input type={'time'} value={time} onChange={(event) => { setTime(event.target.value); setError('') }} required /></label>
  ...
  return <form id={'entry-form'} className={'entry-form card'} onSubmit={submit}>
    <div className={'form-grid'}>{dateField}{timeField}</div>
