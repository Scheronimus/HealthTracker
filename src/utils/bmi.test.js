...
import { bmiStatus, bmiWeightBands, calculateBmi, healthyWeightRange } from './bmi.js'
...
describe('BMI', () => {
  ...
  it('converts all six WHO categories into weight bands', () => {
    const bands = bmiWeightBands(180)
    expect(bands.map(({ key }) => key)).toEqual(['bmiBelow', 'bmiWithin', 'bmiAbove', 'bmiClass1', 'bmiClass2', 'bmiClass3'])
    expect(bands[1].minKg).toBeCloseTo(59.94)
    expect(bands[1].maxKg).toBe(81)
    expect(bands.at(-1).maxKg).toBe(Infinity)
  })
